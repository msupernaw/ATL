...
/* 
 * File:   HashContainers.hpp
 * Author: matthewsupernaw
 *
 * Created on January 20, 2017, 2:52 PM
 */

#ifndef HASHCONTAINERS_HPP
#define HASHCONTAINERS_HPP
...
namespace std {
    ...
    template <typename T>
    struct hash<emlib::HashCache<T> > {

        const size_t operator()(const emlib::HashCache<T>& v) const {
            return v.hash();
        }
    };
}
#endif /* HASHCONTAINERS_HPP */
