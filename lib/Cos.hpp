...
namespace atl {
    ...
    struct Cos : public ExpressionBase<REAL_T, Cos<REAL_T, EXPR> > {
        ...
        inline const REAL_T EvaluateFirstDerivative(uint32_t x) const {
            return -1.0 * expr_m.EvaluateFirstDerivative(x) * std::sin(expr_m.GetValue());
        }
        ...
        inline REAL_T EvaluateSecondDerivative(uint32_t x, uint32_t y) const {
            return ((-1.0 * std::cos(expr_m.GetValue()) * expr_m.EvaluateFirstDerivative(x) * expr_m.EvaluateFirstDerivative(y))
                    - (1.0 * std::sin(expr_m.GetValue()) * expr_m.EvaluateSecondDerivative(x, y)));
        }
        ...
        inline REAL_T EvaluateThirdDerivative(uint32_t x, uint32_t y, uint32_t z) const {
            return std::sin(expr_m.GetValue())*(expr_m.EvaluateFirstDerivative(x))*
                    (expr_m.EvaluateFirstDerivative(y))*(expr_m.EvaluateFirstDerivative(z))
                    - std::cos(expr_m.GetValue())*(expr_m.EvaluateSecondDerivative(x, y))
                    *(expr_m.EvaluateFirstDerivative(z)) - std::cos(expr_m.GetValue())
                    *(expr_m.EvaluateFirstDerivative(x))*(expr_m.EvaluateSecondDerivative(y, z))
                    - std::cos(expr_m.GetValue())*(expr_m.EvaluateSecondDerivative(x, z))
                    *(expr_m.EvaluateFirstDerivative(y)) -
                    std::sin(expr_m.GetValue())*(expr_m.EvaluateThirdDerivative(x, y, z));
        }
        ...
        inline const REAL_T EvaluateFirstDerivativeAt(uint32_t x, size_t i, size_t j = 0) const {
            return -1.0 * expr_m.EvaluateFirstDerivativeAt(x, i, j) * std::sin(expr_m.GetValue(i, j));
        }
        ...
        inline REAL_T EvaluateSecondDerivativeAt(uint32_t x, uint32_t y, size_t i, size_t j = 0) const {
            return ((-1.0 * std::cos(expr_m.GetValue(i, j)) * expr_m.EvaluateFirstDerivativeAt(x, i, j) * expr_m.EvaluateFirstDerivativeAt(y, i, j))
                    - (1.0 * std::sin(expr_m.GetValue(i, j)) * expr_m.EvaluateSecondDerivativeAt(x, y, i, j)));
        }
        ...
        inline REAL_T EvaluateThirdDerivativeAt(uint32_t x, uint32_t y, uint32_t z, size_t i, size_t j = 0) const {
            return std::sin(expr_m.GetValue(i, j))*(expr_m.EvaluateFirstDerivativeAt(x, i, j))*
                    (expr_m.EvaluateFirstDerivativeAt(y, i, j))*(expr_m.EvaluateFirstDerivativeAt(z, i, j))
                    - std::cos(expr_m.GetValue(i, j))*(expr_m.EvaluateSecondDerivativeAt(x, y, i, j))
                    *(expr_m.EvaluateFirstDerivativeAt(z, i, j)) - std::cos(expr_m.GetValue(i, j))
                    *(expr_m.EvaluateFirstDerivativeAt(x, i, j))*(expr_m.EvaluateDerivative(y, z, i, j))
                    - std::cos(expr_m.GetValue(i, j))*(expr_m.EvaluateSecondDerivativeAt(x, z, i, j))
                    *(expr_m.EvaluateFirstDerivativeAt(y, i, j)) -
                    std::sin(expr_m.GetValue(i, j))*(expr_m.EvaluateThirdDerivativeAt(x, y, z, i, j));
        }
