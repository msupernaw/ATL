...
struct less_variable_info {
	...
	bool operator()(const atl::intrusive_ptr<VariableInfo<REAL_T> > &lhs,
			const atl::intrusive_ptr<VariableInfo<REAL_T> > &rhs) const {
...
#warning use  std::unordered_map  here?????
	...
	StackEntry(const StackEntry<REAL_T> &other) :
			exp(other.exp), w(other.w), wv(other.wv), min_id(other.min_id), max_id(
					other.max_id), is_nl(other.is_nl), ids(other.ids), nl_ids(
					other.nl_ids), pushed_ids(other.pushed_ids), id_list(
					other.id_list), first(other.first), second(other.second), third(
					other.third), taylor_coeff(other.taylor_coeff) {
	}
...
std::ostream& operator<<(std::ostream &out, const StackEntry<T> &entry) {
	...
	if (entry.third.size()
			== entry.first.size() * entry.first.size() * entry.first.size()) {
		out << "local jerk[ \n";
		for (int i = 0; i < entry.first.size(); i++) {
			out << "[";
			for (int j = 0; j < entry.first.size(); j++) {
				...
				for (int k = 0; k < entry.first.size(); k++) {
					out
							<< entry.third[i * entry.first.size()
									* entry.first.size()
									+ j * entry.first.size() + k] << " ";
				}
				out << "] ";
			}
			out << "]\n";
...
public:
	...
	typename std::unordered_map<uint32_t,
			typename atl::StackEntry<REAL_T>::vi_storage> forward_dependencies; //ids used in forward mode derivatives
	...
	size_t GetBlock(size_t size) {
		...
		if (stack_current + size + 2 >= this->stack.size()) {
			std::cout << "Resizing tape entry vector to size "
					<< (this->stack.size() + size * 2) << "...";
	...
	inline const size_t NextIndex() {
		...
		if (stack_current + 2 >= this->stack.size()) {
			std::cout << "Resizing Stack Entry vector at size " << stack.size()
					<< "." << std::endl;
	...
	const typename atl::StackEntry<REAL_T>::vi_storage IndependentList() {
		...
		for (int i = 0; i < this->stack_current; i++) {
			for (it = this->stack[i].ids.begin();
					it != this->stack[i].ids.end(); ++it) {
				if ((*it)->count == 1) {
					ilist.insert((*it));
				}
			}
	...
	void Accumulate() {
		...
		std::vector<REAL_T> &W = this->taylor_coeff[this->stack[(stack_current
				- 1)].w->id];
		if (W.size() < this->taylor_order)
			W.resize(taylor_order + 1);
		...
		for (int i = (stack_current - 1); i >= 0; i--) {
			...
			for (vit = this->stack[i].ids.begin();
				...
				std::vector<REAL_T> &vbar =
						this->stack[i].taylor_coeff[(*vit)->id];
				assert(vbar.size() == (taylor_order + 1));

				for (int j = 1; j <= taylor_order; j++) {
					for (int k = j; k <= taylor_order; k++) {
						v[k] += (WW[k] * vbar[k])
								/ (1.0 / (REAL_T) factorial(k - 1));
					}
				}
			}
		}
	...
	void DynamicReverse(first_order_container &derivatives) {
		derivatives[this->stack[(stack_current - 1)].w->id] =
	...
	void AccumulateFirstOrder(bool reset = true) {
		if (recording) {
			...
			if (reset) {
				this->first_order_derivatives[this->stack[(stack_current - 1)].w->id] =
						static_cast<REAL_T>(1.0);
			}
			REAL_T w = static_cast<REAL_T>(0.0);
			typename atl::StackEntry<REAL_T>::vi_iterator vit;
	...
	inline void PushLive(
			std::unordered_map<uint32_t,
					typename atl::StackEntry<REAL_T>::vi_storage> &live_set,
...
//					static_cast<REAL_T>(1.0);
	...
	void AccumulateThirdOrder() {
		...
		if (recording) {
			...
			//initialize w
			this->Reference(stack[stack_current - 1].w->id) =
					static_cast<REAL_T>(1.0);
			unsigned rows = 0; //the size of the local derivatives, anything higher was pushed from previous calculation
			...
			for (int i = (stack_current - 1); i >= 0; i--) {
				...
				std::vector<bool> gh(ID_LIST_SIZE * ID_LIST_SIZE, false);
				std::vector<bool> ghh(
						ID_LIST_SIZE * ID_LIST_SIZE * ID_LIST_SIZE, false);
				vij.resize(ID_LIST_SIZE);
				...
				if (w != REAL_T(0.0)) {
					for (unsigned j = 0; j < rows; j++) {
						this->Reference(current_entry.id_list[j]->id) += w
								* current_entry.first[j];
					}
				}
				VariableInfoPtr vk;
				VariableInfoPtr vj;
...
#pragma unroll
					for (unsigned k = j; k < ID_LIST_SIZE; k++) {
						...
						vijk_[(j * ID_LIST_SIZE) + k] = Value(vi->id, vj->id,
								vk->id);

						if (vijk_[(j * ID_LIST_SIZE) + k]
								!= static_cast<REAL_T>(0.0)) {
							Reference(vi->id, vj->id, vk->id) =
									static_cast<REAL_T>(0.0);
						}

						vijk_[(k * ID_LIST_SIZE) + j] = vijk_[(j * ID_LIST_SIZE)
								+ k]; // dijk;
					}
				}
...
#pragma unroll
					for (int k = j; k < rows; k++) {
						...
						for (int l = k; l < rows; l++) {
							...
							entry_3 += (d3 * w) + (pjl * vij[k])
									+ (dk * pjl * hii)
									+ (dl * vijk_[(j * ID_LIST_SIZE + k)])
									+ (pkl * vij[j]) + (dk * dl * viij_[j])
									+ (pjk * dl * hii);
...
#pragma unroll
							for (int l = k; l < ID_LIST_SIZE; l++) {
								...
								if (entry_3 != static_cast<REAL_T>(0.0)) {
									VariableInfoPtr vk =
											current_entry.id_list[k];
									VariableInfoPtr vl =
											current_entry.id_list[l];
									Reference(vj->id, vk->id, vl->id) +=
											entry_3;
									this->PushLive(live_sets, vj, vl);
									this->PushLive(live_sets, vj, vk);
									//                                        this->PushLive(live_sets, vk, vj);
	...
	void DumpDerivatives() {
		...
		for (st = this->second_order_derivatives.begin();
				st != this->second_order_derivatives.end(); ++st) {
