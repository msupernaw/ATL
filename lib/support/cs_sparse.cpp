...
template<typename Type>
static csi cs_diag(csi i, csi j, Type aij, void *other) {
    return (i != j);
}

/* p = amd(A+A') if symmetric is true, or amd(A'A) otherwise */
template<typename Type>
csi *cs_amd(csi order, const cs_sparse<Type> *A) /* order 0:natural, 1:Chol, 2:LU, 3:QR */ {
    cs_sparse<Type> *C, *A2, *AT;
...
cs_numeric<Type> *cs_chol(const cs_sparse<Type> *A, const cs_symbolic<Type> *S) {
    ...
    pinv = S->pinv;
    parent = S->parent;
    C = pinv ? cs_symperm(A, pinv, 1) : ((cs_sparse<Type>*) A);
    ...
    Ci = C->i;
    Cx = C->x;
    N->L = L = cs_spalloc<Type>(n, n, cp [n], 1, 0); /* allocate result */
...
cs_sparse<Type> *cs_compress(const cs_sparse<Type> *T) {
    ...
    Tx = T->x;
    nz = T->nz;
    C = cs_spalloc<Type>(m, n, nz, Tx != NULL, 0); /* allocate result */
    ...
    cs_cumsum<Type>(Cp, w, n); /* column pointers */
    for (k = 0; k < nz; k++) {
...
template<typename Type>
static void init_ata(cs_sparse<Type> *AT, const csi *post, csi *w, csi **head, csi **next) {
    ...
    for (k = 0; k < n; k++) {
        ...
        for (J = HEAD(k, j); J != -1; J = NEXT(J)) /* J=j for LL'=A case */ {
            for (p = ATp [J]; p < ATp [J + 1]; p++) {
                i = ATi [p];
                q = cs_leaf<Type>(i, j, first, maxfirst, prevleaf, ancestor, &jleaf);
...
Type cs_cumsum(csi *p, csi *c, csi n) {
    ...
    for (i = 0; i < n; i++) {
        p [i] = nz;
        nz += c [i];
        nz2 += c [i]; /* also in Type to avoid csi overflow */
...
template<typename Type>
static csi cs_bfs(const cs_sparse<Type> *A, csi n, csi *wi, csi *wj, csi *queue,
    ...
    C = (mark == 1) ? ((cs_sparse<Type>*) A) : cs_transpose(A, 0);
...
template<typename Type>
static csi cs_rprune(csi i, csi j, Type aij, void *other) {
    csi *rr = (csi *) other;
...
cs_dmperm_results<Type> *cs_dmperm(const cs_sparse<Type> *A, csi seed) {
    ...
    cs_sparse<Type> *C;
    cs_dmperm_results<Type> *D, *scc;
    /* --- Maximum matching ------------------------------------------------- */
    if (!CS_CSC(A)) return (NULL); /* check inputs */
    m = A->m;
    n = A->n;
    D = cs_dalloc<Type>(m, n); /* allocate result */
    if (!D) return (NULL);
    p = D->p;
    q = D->q;
    r = D->r;
    ...
    cs_matched<Type>(n, wj, imatch, p, q, cc, rr, 1, 1); /* set R1 and C1 */
    cs_matched<Type>(n, wj, imatch, p, q, cc, rr, 2, -1); /* set R2 and C2 */
    cs_matched<Type>(n, wj, imatch, p, q, cc, rr, 3, 3); /* set R3 and C3 */
...
template<typename Type>
static csi cs_tol(csi i, csi j, Type aij, void *tol) {
    return (std::fabs(aij) > *((Type *) tol));
}
...
template<typename Type>
static csi cs_nonzero(csi i, csi j, Type aij, void *other) {
    return (aij != 0);
}
...
csi *cs_etree(const cs_sparse<Type> *A, csi ata) {
    ...
    if (!w || !parent) return (cs_idone<long>(parent, NULL, w, 0));
    ancestor = w;
    prev = w + n;
    ...
    return (cs_idone<long>(parent, NULL, w, 1));
}
...
cs_sparse<Type> *cs_load(FILE *f) {
    Type i, j; /* use Type for integers to avoid csi conflicts */
    Type x;
    cs_sparse<Type> *T;
    if (!f) return (NULL); /* check inputs */
    T = cs_spalloc<Type>(0, 0, 1, 1, 1); /* allocate result */
...
csi cs_ltsolve(const cs_sparse<Type> *L, Type *x, bool concurrent) {
    ...
    if (concurrent && (n - 1) > std::thread::hardware_concurrency()) {
...
template<typename Type>
static void cs_augment(csi k, const cs_sparse<Type> *A, csi *jmatch, csi *cheap, csi *w,
...
template<typename Type>
csi *cs_maxtrans(const cs_sparse<Type> *A, csi seed) /*[jmatch [0..m-1]; imatch [0..n-1]]*/ {
    ...
    if (k == CS_MIN(m, n)) /* quick return if diagonal zero-free */ {
        ...
        return (cs_idone<long>(jimatch, NULL, NULL, 1));
    }
    for (i = 0; i < m; i++) m2 += w [i];
    C = (m2 < n2) ? cs_transpose(A, 0) : ((cs_sparse<Type>*) A); /* transpose if needed */
    ...
    q = cs_randperm<Type>(n, seed); /* q = random permutation */
...
csi *cs_post(const csi *parent, csi n) {
    ...
    if (!w || !post) return (cs_idone<long>(post, NULL, w, 0));
    head = w;
    next = w + n;
    stack = w + 2 * n;
    ...
    return (cs_idone<long>(post, NULL, w, 1)); /* success; free w, return post */
}
...
csi cs_print(const cs_sparse<Type> *A, csi brief) {
    ...
    if (nz < 0) {
        printf("%g-by-%g, nzmax: %g nnz: %g, 1-norm: %g\n", (Type) m,
                (Type) n, (Type) nzmax, (Type) (Ap [n]), cs_norm(A));
        for (j = 0; j < n; j++) {
            printf("    col %g : locations %g to %g\n", (Type) j,
                    (Type) (Ap [j]), (Type) (Ap [j + 1] - 1));
            for (p = Ap [j]; p < Ap [j + 1]; p++) {
                printf("      %g : %g\n", (Type) (Ai [p]), Ax ? Ax [p] : 1);
                if (brief && p > 20) {
                    printf("  ...\n");
...
csi cs_qrsol(csi order, const cs_sparse<Type> *A, Type *b) {
    ...
    if (m >= n) {
        ...
        x = (Type*) cs_calloc(S ? S->m2 : 1, sizeof (Type)); /* get workspace */
        ok = (S && N && x);
        if (ok) {
...
template<typename Type>
cs_dmperm_results<Type> *cs_scc(cs_sparse<Type> *A) /* matrix A temporarily modified, then restored */ {
    csi n, i, k, b, nb = 0, top, *xi, *pstack, *p, *r, *Ap, *ATp, *rcopy, *Blk;
    cs_sparse<Type> *AT;
    cs_dmperm_results<Type> *D;
    if (!CS_CSC(A)) return (NULL); /* check inputs */
    n = A->n;
    Ap = A->p;
    D = cs_dalloc<Type>(n, 0); /* allocate result */
    AT = cs_transpose(A, 0); /* AT = A' */
...
template<typename Type>
csi cs_spsolve(cs_sparse<Type> *G, const cs_sparse<Type> *B, csi k, csi *xi, Type *x, const csi *pinv,
        csi lo) {
...
template<typename Type>
static csi cs_vcount(const cs_sparse<Type> *A, cs_symbolic<Type> *S) {
...
cs_symbolic<Type> *cs_sqr(csi order, const cs_sparse<Type> *A, csi qr) {
    ...
    if (qr) /* QR symbolic analysis */ {
        cs_sparse<Type> *C = order ? cs_permute(A, NULL, S->q, 0) : ((cs_sparse<Type>*) A);
...
cs_sparse<Type> *cs_symperm(const cs_sparse<Type> *A, const csi *pinv, csi values) {
    ...
    Ai = A->i;
    Ax = A->x;
    C = cs_spalloc<Type>(n, n, Ap [n], values && (Ax != NULL), 0); /* alloc result*/
    w = (csi*) cs_calloc(n, sizeof (csi)); /* get workspace */
    if (!C || !w) return (cs_done(C, w, NULL, 0)); /* out of memory */
    Cp = C->p;
    Ci = C->i;
    ...
    cs_cumsum<Type>(Cp, w, n); /* compute column pointers of C */
    for (j = 0; j < n; j++) {
        j2 = pinv ? pinv [j] : j; /* column j of A is column j2 of C */
        for (p = Ap [j]; p < Ap [j + 1]; p++) {
            i = Ai [p];
...
cs_sparse<Type> *cs_transpose(const cs_sparse<Type> *A, csi values) {
    ...
    Ai = A->i;
    Ax = A->x;
    C = cs_spalloc<Type>(n, m, Ap [n], values && Ax, 0); /* allocate result */
    w = (csi*) cs_calloc(m, sizeof (csi)); /* get workspace */
    if (!C || !w) return (cs_done(C, w, NULL, 0)); /* out of memory */
    Cp = C->p;
    Ci = C->i;
    ...
    cs_cumsum<Type>(Cp, w, m); /* row pointers */
    for (j = 0; j < n; j++) {
        for (p = Ap [j]; p < Ap [j + 1]; p++) {
...
csi cs_updown(cs_sparse<Type> *L, csi sigma, const cs_sparse<Type> *C, const csi *parent) {
    ...
    Type *Lx, *Cx, alpha, beta = 1, delta, gamma, w1, w2, *w, beta2 = 1;
    ...
    w = (Type*) cs_malloc(n, sizeof (Type)); /* get workspace */
    if (!w) return (0); /* out of memory */
    f = Ci [p];
...
cs_sparse<Type> *cs_spalloc(csi m, csi n, csi nzmax, csi values, csi triplet) {
    cs_sparse<Type> *A = (cs_sparse<Type>*) cs_calloc(1, sizeof (cs_sparse<Type>)); /* allocate the cs struct */
...
csi cs_sprealloc(cs_sparse<Type> *A, csi nzmax) {
    ...
    if (A->x) A->x = (Type*) cs_realloc(A->x, nzmax, sizeof (Type), &okx);
    ok = (oki && okj && okx);
    if (ok) A->nzmax = nzmax;
    return (ok);
}
...
cs_sparse<Type> *cs_spfree(cs_sparse<Type> *A) {
    ...
    cs_free(A->i);
    cs_free(A->x);
    return ((cs_sparse<Type>*) cs_free(A)); /* free the cs struct and return NULL */
}
...
cs_numeric<Type> *cs_nfree(cs_numeric<Type> *N) {
    ...
    cs_free(N->pinv);
    cs_free(N->B);
    return ((cs_numeric<Type>*) cs_free(N)); /* free the csn struct and return NULL */
}
...
cs_symbolic<Type> *cs_sfree(cs_symbolic<Type> *S) {
    ...
    cs_free(S->cp);
    cs_free(S->leftmost);
    return ((cs_symbolic<Type>*) cs_free(S)); /* free the css struct and return NULL */
}
...
cs_dmperm_results<Type> *cs_dalloc(csi m, csi n) {
    cs_dmperm_results<Type> *D;
    D = (cs_dmperm_results<Type>*) cs_calloc(1, sizeof (cs_dmperm_results<Type>));
    if (!D) return (NULL);
...
cs_dmperm_results<Type> *cs_dfree(cs_dmperm_results<Type> *D) {
    ...
    cs_free(D->r);
    cs_free(D->s);
    return ((cs_dmperm_results<Type>*) cs_free(D)); /* free the csd struct and return NULL */
}
