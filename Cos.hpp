...
namespace atl {
    ...
    struct Cos : public ExpressionBase<REAL_T, Cos<REAL_T, EXPR> > {
        ...
        inline const REAL_T EvaluateDerivative(uint32_t id, size_t i, size_t j = 0) const {
            return -1.0 * expr_m.EvaluateDerivative(id, i, j) * std::sin(expr_m.GetValue(i, j));
        }

        inline REAL_T EvaluateDerivative(uint32_t a, uint32_t b, size_t i, size_t j = 0) const {
            return ((-1.0 * std::cos(expr_m.GetValue(i, j)) * expr_m.EvaluateDerivative(a, i, j) * expr_m.EvaluateDerivative(b, i, j))
                    - (1.0 * std::sin(expr_m.GetValue(i, j)) * expr_m.EvaluateDerivative(a, b, i, j)));
        }
