...
namespace atl {
    ...
    struct Tanh : public ExpressionBase<REAL_T, Tanh<REAL_T, EXPR> > {
        ...
        inline const REAL_T EvaluateDerivative(uint32_t id, size_t i, size_t j = 0) const {
            ...
            return expr_m.EvaluateDerivative(id, i, j) * sech*sech;
        }

        inline REAL_T EvaluateDerivative(uint32_t a, uint32_t b, size_t i, size_t j = 0) const {
            REAL_T sech = 1.0 / std::cosh(expr_m.GetValue(i, j));
            REAL_T sech2 = sech*sech;
            return sech2 * expr_m.EvaluateDerivative(a, b, i, j) - 2.0 * sech2 * this->GetValue(i, j) * expr_m.EvaluateDerivative(a, i, j) * expr_m.EvaluateDerivative(b, i, j);
