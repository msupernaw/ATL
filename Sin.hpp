...
namespace atl {
    ...
    struct Sin : public ExpressionBase<REAL_T, Sin<REAL_T, EXPR> > {
        ...
        inline const REAL_T EvaluateDerivative(uint32_t x) const {
            return expr_m.EvaluateDerivative(x) * std::cos(expr_m.GetValue());
        }
        ...
        inline REAL_T EvaluateDerivative(uint32_t x, uint32_t y) const {
            return (std::cos(expr_m.GetValue()) * expr_m.EvaluateDerivative(x, y))-
                    std::sin(expr_m.GetValue()) * expr_m.EvaluateDerivative(x) * expr_m.EvaluateDerivative(y);
        }
        ...
        inline const REAL_T EvaluateDerivative(uint32_t x, size_t i, size_t j = 0) const {
            return expr_m.EvaluateDerivative(x, i, j) * std::cos(expr_m.GetValue(i, j));
        }
        ...
        inline REAL_T EvaluateDerivative(uint32_t x, uint32_t y, size_t i, size_t j = 0) const {
            return (std::cos(expr_m.GetValue(i, j)) * expr_m.EvaluateDerivative(x, y, i, j))-
                    std::sin(expr_m.GetValue(i, j)) * expr_m.EvaluateDerivative(x, i, j) * expr_m.EvaluateDerivative(y, i, j);
        }
